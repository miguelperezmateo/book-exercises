...
words <- c("Hola","Botón","Ventana","Asignatura","Cuaderno","Motosierra")
...
# "g" through "m"
g_m_words <- words[substring(words,1,1)<="m" & substring(words,1,1)>="g"]
...
word_bin <- function(vector,letter1,letter2){
  return(vector[substring(vector,1,1)<=letter2 & substring(words,1,1)>=letter1])
}
