...
sentence <- tolower("this is a sentence created for an exercise")
...
splitSentence <- strsplit(sentence,"")

# Extract the vector of letters from the resulting list
vectorsplit <- splitSentence[[1]]

# Use the `unique()` function to get a vector of unique letters
uniq <- unique(vectorsplit)
...
sapply(uniq,count_occurrences,vectorsplit)
